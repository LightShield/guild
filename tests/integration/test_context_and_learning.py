...
class TestLearningInjection:
    ...
    async def test_high_confidence_learnings_appear_in_injection(self, storage: Storage) -> None:
        ...
        await storage.add_learning(
            LearningRecord(category="pattern", content="Always use guard clauses", confidence=0.8)
        )
        await storage.add_learning(
            LearningRecord(category="tool_tip", content="Run ruff before commit", confidence=0.7)
        )
...
class TestLearningDecay:
    ...
    async def test_decay_reduces_stale_learning_confidence(self, storage: Storage) -> None:
        ...
        lid = await storage.add_learning(
            LearningRecord(category="pattern", content="Stale pattern", confidence=0.7)
        )
    ...
    async def test_recently_validated_not_decayed(self, storage: Storage) -> None:
        ...
        lid = await storage.add_learning(
            LearningRecord(category="pattern", content="Fresh pattern", confidence=0.7)
        )
        await storage.validate_learning(lid)
    ...
    async def test_decayed_learning_drops_below_injection_threshold(self, storage: Storage) -> None:
        ...
        lid = await storage.add_learning(
            LearningRecord(category="pattern", content="Was good once", confidence=0.55)
        )
        old_date = (datetime.now(UTC) - timedelta(days=60)).isoformat()
        assert storage._db is not None
        await storage._db.execute(
